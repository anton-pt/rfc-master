...
async function demonstrateLeadAgentTools() {
  ...
  console.log(
    '1️⃣ Simulating user request: "Create an RFC for migrating our auth system to OAuth"\n'
  );
...
You have access to tools for RFC management. Use them to handle user requests effectively.`,
        },
        {
          role: "user",
          content:
            "Create an RFC for migrating our authentication system from custom tokens to OAuth 2.0. Include problem statement, solution, and implementation sections. Then request reviews from backend and security teams.",
        },
      ],
      maxTokens: 2000,
    });
  ...
  const createResult = await leadAgentTools.createRFCDocument.execute({
    ...
    description:
      "Replace our custom token-based authentication with industry-standard OAuth 2.0",
  ...
  if ("error" in searchResult) {
    ...
    console.log(
      `✅ Found ${searchResult.results.length} relevant code snippets:`
    );
...
- Store tokens with appropriate encryption`,
  ...
  const reviewResult = await leadAgentTools.requestReview.execute({
    ...
    specificConcerns:
      "Focus on security implications, API breaking changes, and client-side integration complexity",
  });
  ...
  if ("error" in reviewResult) {
    ...
    console.log(
      `   - Reviewers assigned: ${reviewResult.reviewersAssigned.length}`
    );
  ...
  const commentResult = await leadAgentTools.addLeadComment.execute({
    ...
    type: "document-level",
    content:
      "This RFC is high priority due to security vulnerabilities in our current custom auth system. Target completion: Q2.",
    category: "context",
  });
  ...
  await domainModel.addComment({
    ...
    commentType: "inline",
    content:
      "Consider using Redis for token storage instead of database tables for better performance",
  ...
  await domainModel.addComment({
    ...
    content:
      "CRITICAL: Must implement rate limiting on OAuth endpoints to prevent abuse. Also need proper token revocation mechanism.",
  });
...
- Add audit logging for all OAuth operations`,
  ...
  const securityComment = commentsResult.comments.find(
    (c) => c.agentType === "security"
  );
  if (securityComment) {
    await leadAgentTools.resolveComment.execute({
      commentId: securityComment.commentId,
      resolution:
        "Added comprehensive rate limiting and token security section addressing all security concerns",
      rfcUpdated: true,
    });
  }
